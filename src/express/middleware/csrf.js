...
export function csrfTokenHandler(req, res) {
  ...
  if (!token) {
    return res.status(msgs[config.app.lang].errors.client.unknown.code).send(msgs[config.app.lang].errors.client.unknown)
  }
...
export function csrfProtection(req, res, next) {
  ...
  if (typeof expected !== 'string' || expected.length === 0) {
    return res.status(msgs[config.app.lang].errors.client.csrfInvalid.code).send(msgs[config.app.lang].errors.client.csrfInvalid)
  }
