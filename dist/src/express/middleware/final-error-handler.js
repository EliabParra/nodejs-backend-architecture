...
export function createFinalErrorHandler({ clientErrors, serverErrors }) {
    return function finalErrorHandler(err, req, res, next) {
        ...
        if (status === 400)
            response = clientErrors.invalidParameters;
        else if (status === 413)
            response = clientErrors.payloadTooLarge ?? clientErrors.unknown;
        else if (status === 401)
            response = serverErrors.unauthorized;
        else if (status === 403)
            response = serverErrors.forbidden;
        else if (status === 404)
            response = serverErrors.notFound;
        else if (status === 503)
            response = clientErrors.serviceUnavailable;
        const rawMessage = typeof err?.message === 'string' ? redactSecretsInString(err.message.trim()) : '';
