...
async function ensureSpaDistPathIfNeeded() {
    const hasPath = (typeof process.env.SPA_DIST_PATH === 'string' &&
        process.env.SPA_DIST_PATH.trim().length > 0) ||
        (typeof config?.app?.spaDistPath === 'string' &&
            String(config.app.spaDistPath).trim().length > 0);
    if (hasPath)
        return;
    const msg = 'SPA mode enabled but SPA_DIST_PATH is missing.\n' +
...
function resolveSpaDistPath() {
    ...
    if (fromConfig && String(fromConfig).trim().length > 0)
        return path.resolve(String(fromConfig));
    // Deliberately no default: keep backend decoupled from any specific frontend repo/framework.
    return null;
}
export async function registerSpaHosting(app) {
    await ensureSpaDistPathIfNeeded();
    const distPath = resolveSpaDistPath();
    if (!distPath) {
        throw new Error('SPA mode enabled but no dist path configured. Set SPA_DIST_PATH (env) or config.app.spaDistPath to a folder containing index.html.');
    }
