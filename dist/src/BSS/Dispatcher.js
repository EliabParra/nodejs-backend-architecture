...
import { createLoginRateLimiter, createToProccessRateLimiter, createAuthPasswordResetRateLimiter, } from '../express/rate-limit/limiters.js';
...
import { isPlainObject, parseLoginBody, parseLoginVerifyBody, parseLogoutBody, parseToProccessBody, } from './helpers/http-validators.js';
...
export default class Dispatcher {
    ...
    async init() {
        ...
        this.app.post('/toProccess', this.toProccessRateLimiter, this.authPasswordResetRateLimiter, this.csrfProtection, this.toProccess.bind(this));
        this.app.post('/login', this.loginRateLimiter, this.csrfProtection, this.login.bind(this));
        this.app.post('/login/verify', this.loginRateLimiter, this.csrfProtection, this.verifyLogin.bind(this));
        ...
        this.app.use(createFinalErrorHandler({
    ...
    async toProccess(req, res) {
        ...
        try {
            ...
            if (parsed.ok === false) {
                return sendInvalidParameters(res, this.clientErrors.invalidParameters, parsed.alerts);
            }
            ...
            effectiveLog.show({
                ...
                msg: `${this.serverErrors.serverError.msg}, /toProccess: ${redactSecretsInString(err?.message || err)}`,
                ctx: {
                    ...
                    durationMs: typeof req.requestStartMs === 'number'
    ...
    async login(req, res) {
        try {
            ...
            effectiveLog.show({
                type: effectiveLog.TYPE_ERROR,
                msg: `${this.serverErrors.serverError.msg}, /login: ${redactSecretsInString(err?.message || err)}`,
                ctx: {
                    requestId: req.requestId,
                    method: req.method,
                    path: req.originalUrl,
    ...
    async verifyLogin(req, res) {
        try {
            ...
            effectiveLog.show({
                type: effectiveLog.TYPE_ERROR,
                msg: `${this.serverErrors.serverError.msg}, /login/verify: ${redactSecretsInString(err?.message || err)}`,
                ctx: {
                    requestId: req.requestId,
                    method: req.method,
                    path: req.originalUrl,
    ...
    async logout(req, res) {
        try {
            ...
            effectiveLog.show({
                type: effectiveLog.TYPE_ERROR,
                msg: `${this.serverErrors.serverError.msg}, /logout: ${redactSecretsInString(err?.message || err)}`,
                ctx: {
                    requestId: req.requestId,
                    method: req.method,
                    path: req.originalUrl,
    ...
    serverOn() {
        ...
        this.server = this.app.listen(config.app.port, () => (this.ctx?.log ?? log).show({
