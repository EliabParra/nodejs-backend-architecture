...
export default class Session {
    ...
    login2StepNewDevice;
    deviceCookieName;
    deviceCookieMaxAgeMs;
    loginChallengeExpiresSeconds;
    ...
    async createSession(req, res) {
        try {
            ...
            const ok = typeof storedHash === 'string' && (await bcrypt.compare(body.password, storedHash));
            if (!ok)
                return res
                    .status(this.clientErrors.usernameOrPasswordIncorrect.code)
                    .send(this.clientErrors.usernameOrPasswordIncorrect);
            ...
            log.show({
                type: log.TYPE_ERROR,
                msg: `${this.serverErrors.serverError.msg}, Session.createSession: ${redactSecretsInString(err?.message || err)}`,
                ctx: {
    ...
    async _startLoginChallenge(req, res, user) {
        const effectiveConfig = this.ctx?.config ?? config;
    ...
    async verifyLoginChallenge(req, res) {
        try {
            if (this.sessionExists(req)) {
                return res
            ...
            if (Number.isFinite(this.loginChallengeMaxAttempts) &&
            ...
            log.show({
                type: log.TYPE_ERROR,
                msg: `${this.serverErrors.serverError.msg}, Session.verifyLoginChallenge: ${redactSecretsInString(err?.message || err)}`,
                ctx: {
                    requestId: req.requestId,
                    method: req.method,
                    path: req.originalUrl,
