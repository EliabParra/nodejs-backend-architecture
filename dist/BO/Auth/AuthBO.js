...
const successMsgs = require('./messages/authSuccessMsgs.json')[config.app.lang];
...
function getRequestContext(params) {
...
export class AuthBO {
    async register(params) {
        try {
            ...
            log.show({
                type: log.TYPE_ERROR,
                msg: msgs[config.app.lang].errors.server.serverError.msg +
                    ', AuthBO.register: ' +
                    errorMessage(err),
            });
    ...
    async verifyEmail(params) {
        try {
            ...
            if (!otp) {
                ...
                try {
                    ...
                    if (active) {
                        ...
                        const maxAttempts = Number(meta?.maxAttempts ?? config?.auth?.emailVerificationMaxAttempts ?? 5);
            ...
            const maxAttempts = getNumber(meta, 'maxAttempts') ??
    ...
    async resetPassword(params) {
        try {
            ...
            if (!AuthValidate.validateToken(token) ||
