...
function resolveMaybeRelative(p: unknown, baseDir: string) {
    const raw = String(p ?? '').trim()
    if (!raw) return null
    return path.isAbsolute(raw) ? raw : path.resolve(baseDir, raw)
}

function ensureFrontendPath(frontendPath: string) {
    if (!frontendPath) throw new Error('FRONTEND_PATH is missing.')
...
async function promptForFrontendPathIfMissing(backendDir: string): Promise<string> {
    ...
    const resolved = resolveMaybeRelative(entered, backendDir)
    if (!resolved) {
        console.error('FRONTEND_PATH is required to run `npm run full`.')
        process.exit(1)
    }
...
function startProcess(name: string, cmd: string, args: string[], options: any) {
...
function splitArgs(raw: unknown) {
...
function startNpmScript(name: string, scriptName: string, cwd: string, extraArgs: string[] = []) {
