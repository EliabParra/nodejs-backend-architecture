...
Options:
  ...
  --createEnterprise      Also create enterprise schema + person table
...
function pgClientConfig(db) {
	...
	if (db.connectionString) {
		return { connectionString: db.connectionString, ssl: db.ssl ? { rejectUnauthorized: false } : false }
	}
	return {
		host: db.host,
		port: db.port,
...
function sqlEnterpriseDemo() {
	return [
		`create schema if not exists enterprise;`,
		`create table if not exists enterprise.person (\n  person_id bigint generated by default as identity primary key,\n  person_na text not null,\n  person_ln text not null\n);`,
		`create index if not exists ix_person_person_na on enterprise.person(person_na);`
	]
}
...
async function ensureProfile(client, profileId) {
	await client.query('insert into security.profile (profile_id) values ($1) on conflict (profile_id) do nothing', [profileId])
}

async function getNextTxFromDb(client) {
	const r = await client.query('select coalesce(max(tx_nu), 0) + 1 as next_tx from security.method')
...
async function registerBOs(client, { repoRoot, profileId, txStart }) {
	...
	for (const bo of bos) {
		...
		for (const methodName of bo.methods) {
			...
			while (attempts < 50) {
				try {
					const { methodId, txNu } = await upsertMethodKeepTx(client, { objectId, methodName, txNu: nextTx })
					...
					if (msg.toLowerCase().includes('uq_method_tx') || msg.toLowerCase().includes('unique') || msg.toLowerCase().includes('duplicate')) {
						nextTx++
						attempts++
						continue
					}
...
async function promptYesNo(rl, question, defaultYes = false) {
	...
	const ans = String(await rl.question(question + suffix)).trim().toLowerCase()
...
async function main() {
	...
	const sessionSchemaOpt = normalizeSchemaName(opts.sessionSchema ?? process.env.SESSION_SCHEMA ?? 'public')
	const sessionTableOpt = normalizeTableName(opts.sessionTable ?? process.env.SESSION_TABLE ?? 'session')
	...
	const createEnterprise = Boolean(opts.createEnterprise)
	...
	if (createEnterprise) sql.push(...sqlEnterpriseDemo())
	...
	try {
		const info = await client.query('select current_database() as db, inet_server_addr() as server_addr, inet_server_port() as server_port')
		...
		if (seedAdmin) {
			...
			if (!adminPassword || String(adminPassword).length < 8) {
				throw new Error('Admin password is required (min 8 chars). Provide --adminPassword or run interactively.')
			}
			...
			console.log(`Seeded admin user: user_id=${userId}, user_na=${adminUser}, profile_id=${profileId}`)
		}

		await client.query('commit')
		console.log('DB init complete.')
		console.log(`Session table: ${(sessionSchemaOpt ?? 'public')}.${(sessionTableOpt ?? 'session')}`)
		if (includeEmail) console.log('Optional column enabled: security.user.user_em')
		if (createEnterprise) console.log('Demo schema enabled: enterprise.person')
		if (registerBo) console.log(`BO methods auto-registered and granted to profile_id=${profileId}`)
	} catch (err) {
