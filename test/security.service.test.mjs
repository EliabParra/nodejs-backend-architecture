...
test('Security.init loads permissions + tx map and sets isReady', async () => {
  await withGlobals(['config', 'msgs', 'log', 'db'], async () => {
    ...
    assert.equal(security.getPermissions({ profile_id: 1, method_na: 'm', object_na: 'o' }), true)
    assert.equal(security.getPermissions({ profile_id: 2, method_na: 'm', object_na: 'o' }), false)
...
test('Security.executeMethod dynamically imports BO and caches the instance', async () => {
  ...
  try {
    ...
    await withGlobals(['config', 'msgs', 'log', 'db'], async () => {
      ...
      const r1 = await security.executeMethod({ object_na: objectName, method_na: 'ping', params: { a: 1 } })
      const r2 = await security.executeMethod({ object_na: objectName, method_na: 'ping', params: { a: 2 } })
...
test('Security.executeMethod returns serverError and logs when BO import fails', async () => {
  await withGlobals(['config', 'msgs', 'log', 'db'], async () => {
    ...
    const r = await security.executeMethod({ object_na: 'DoesNotExist', method_na: 'nope', params: {} })
